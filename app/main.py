...
if _MODELS_DIR.is_dir():
    app.mount(
        "/3dModels",
        StaticFiles(directory=str(_MODELS_DIR), html=False),
        name="3dModels",
    )
else:
...
async def health_check():
    ...
    glb_root = _MODELS_DIR / "3d model"
    glb_count = 0
    if glb_root.is_dir():
        glb_count = sum(1 for _ in glb_root.rglob("*.glb"))
    return {
        "status": "healthy",
        "service": "VTailor Backend",
        "glb_models": glb_count,
        "glb_static_mount": "/3dModels",
    }
