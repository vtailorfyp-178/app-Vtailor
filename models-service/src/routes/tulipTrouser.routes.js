...
const CASUAL_CATEGORIES = [{ id: 'casual', name: 'Casual dress' }];

const TROUSER_SHIRT_SUBCATEGORIES = [{ id: 'trouser-shirt', name: 'Trouser shirt' }];

const TULIP_TROUSER_VARIATIONS = [
  {
    id: 'tulip-trouser',
    name: 'Tulip trouser',
    thumbnail: null,
...
const TULIP_TROUSER_COLOR_SHADE_IDS = [
  'red-classic',
  'pink-rani',
  'blue-royal',
  'green-mint',
...
router.get('/variations', (_req, res) => {
  ...
  res.json({
    ...
    variations: TULIP_TROUSER_VARIATIONS,
  });
});
...
router.get('/colors', (_req, res) => {
  res.set('Cache-Control', 'public, max-age=600');
  res.json({
    colorShadeIds: TULIP_TROUSER_COLOR_SHADE_IDS,
    runtimeTint: true,
  });
});
...
/**
 * GET /tulip-trouser/model?neck=round&sleeve=full
 */
