/**
 * Upload all tulip-trouser optimized GLBs + MongoDB metadata.
 * Prerequisites:
 *   1. Place source GLBs in `3d model/3d trouser shirt/tulip-trouser/original/`
 *   2. Run: cd app-vTailor-frontend/app-vTailor && npm run compress:tulip-trouser
 *   3. Run: npm run upload:tulip-trouser
...
} = require('../config/tulipTrouserCatalog');

const OPT_DIR = path.join(MODEL_ROOT_PATH, '3d trouser shirt', 'tulip-trouser', 'optimized');
...
async function main() {
  if (!fs.existsSync(OPT_DIR)) {
    ...
    console.error('Run compress:tulip-trouser first.');
    process.exit(1);
  }
  ...
  const variants = listTulipTrouserVariants();
  const catalogRows = [];
  let uploaded = 0;
  let skipped = 0;
  ...
  const outFile = path.join(outDir, 'uploadTulipTrouserResults.json');
